...
public class GetEquipmentsQueryHandler(IUnitOfWork unitOfWork, ICacheService cacheService) : IRequestHandler<GetEquipmentsQuery, PagedResult<EquipmentDto>>
{
    public async Task<PagedResult<EquipmentDto>> Handle(GetEquipmentsQuery request, CancellationToken cancellationToken)
    {
        ...
        var cachedResult = await cacheService.GetAsync<PagedResult<EquipmentDto>>(cacheKey, cancellationToken);
        if (cachedResult != null)
        {
            return cachedResult;
        }

        var (items, totalCount) = await unitOfWork.Equipments.GetPagedAsync(
            request.PageNumber,
            request.PageSize,
            request.Type,
            request.Status,
